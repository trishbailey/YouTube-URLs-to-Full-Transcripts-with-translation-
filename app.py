...
with tab_csv:
    st.markdown(
        "Upload a CSV with YouTube URLs. The app will look for a column named **`url`** or **`URL`**; "
        "if not found it will use the **first column**."
    )
    ...
    if uploaded_file:
        try:
            ...
            # Find the URL column
            url_col = next(
                (c for c in df_upload.columns if c.strip().lower() == "url"),
                df_upload.columns[0],
            )
            ...
            with st.expander("Preview loaded URLs"):
                st.dataframe(df_upload[[url_col]].head(20), use_container_width=True)
        except Exception as e:
